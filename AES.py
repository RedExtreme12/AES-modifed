...
def pad(plaintext):
    ...
    padding = b'\x80' + bytes([0] * (padding_len - 1))
    return plaintext + padding


def unpad(plaintext):
    for idx, byte in enumerate(reversed(plaintext)):
        pad_size = 0
        if bytes([byte]) == b'\x80':
            pad_size = idx + 1
            break
...
class AES:
    """
    Class for AES-128 encryption with CBF mode and ISO.
    ...
    def decrypt_cfb(self, ciphertext, iv):
        ...
        for ciphertext_block in split_blocks(ciphertext):
            ...
            prev_ciphertext = ciphertext_block  # ciphertext
